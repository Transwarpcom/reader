...
public class PackageDocumentMetadataWriter extends PackageDocumentBase {
   public static void writeMetaData(EpubBook book, XmlSerializer serializer) throws IllegalArgumentException, IllegalStateException, IOException {
      ...
      if (book.getMetadata().getOtherProperties() != null) {
         ...
         while(var2.hasNext()) {
            Entry<QName, String> mapEntry = (Entry)var2.next();
            serializer.startTag(((QName)mapEntry.getKey()).getNamespaceURI(), "meta");
            serializer.attribute("", "property", ((QName)mapEntry.getKey()).getLocalPart());
            serializer.text((String)mapEntry.getValue());
            serializer.endTag(((QName)mapEntry.getKey()).getNamespaceURI(), "meta");
         }
      }
