...
final class SemaphoreImpl implements Semaphore {
    ...
    public SemaphoreImpl(int permits, int acquiredPermits) {
        ...
        boolean z = 0 <= acquiredPermits && acquiredPermits <= this.permits;
        if (!z) {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cancellableContinuation) {
        Object objM4335constructorimpl;
        Object s$iv;
        boolean z;
        Segment curTail = (SemaphoreSegment) this.tail;
        long enqIdx = enqIdx$FU.getAndIncrement(this);
        long id$iv = enqIdx / SemaphoreKt.SEGMENT_SIZE;
        do {
            Segment $this$findSegmentInternal$iv$iv = curTail;
            Segment cur$iv$iv = $this$findSegmentInternal$iv$iv;
            while (true) {
                if (cur$iv$iv.getId() >= id$iv && !cur$iv$iv.getRemoved()) {
                    objM4335constructorimpl = SegmentOrClosed.m4335constructorimpl(cur$iv$iv);
                    break;
                }
                ...
                if (it$iv$iv$iv == ConcurrentLinkedListKt.CLOSED) {
                    objM4335constructorimpl = SegmentOrClosed.m4335constructorimpl(ConcurrentLinkedListKt.CLOSED);
                    break;
                }
            ...
            s$iv = objM4335constructorimpl;
            if (SegmentOrClosed.m4330isClosedimpl(s$iv)) {
                break;
            }
            Segment to$iv$iv = SegmentOrClosed.m4331getSegmentimpl(s$iv);
            while (true) {
        ...
        SemaphoreSegment segment = (SemaphoreSegment) SegmentOrClosed.m4331getSegmentimpl(s$iv);
        int i = (int) (enqIdx % SemaphoreKt.SEGMENT_SIZE);
        if (segment.acquirers.compareAndSet(i, null, cancellableContinuation)) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        Object cellState = segment.acquirers.getAndSet(i, SemaphoreKt.PERMIT);
        if (cellState == null) {
        ...
        return tryResumeAcquire((CancellableContinuation) cellState);
    }
