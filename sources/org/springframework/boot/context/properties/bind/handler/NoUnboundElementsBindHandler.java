...
/* loaded from: reader.jar:BOOT-INF/lib/spring-boot-2.1.6.RELEASE.jar:org/springframework/boot/context/properties/bind/handler/NoUnboundElementsBindHandler.class */
public class NoUnboundElementsBindHandler extends AbstractBindHandler {
    ...
    private boolean isUnbound(ConfigurationPropertyName name, ConfigurationPropertyName candidate) {
        if (name.isAncestorOf(candidate) && !this.boundNames.contains(candidate) && !isOverriddenCollectionElement(candidate)) {
            return true;
        }
        return false;
    }
