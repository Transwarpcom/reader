...
/* loaded from: reader.jar:BOOT-INF/lib/spring-boot-autoconfigure-2.1.6.RELEASE.jar:org/springframework/boot/autoconfigure/web/embedded/TomcatWebServerFactoryCustomizer.class */
public class TomcatWebServerFactoryCustomizer implements WebServerFactoryCustomizer<ConfigurableTomcatWebServerFactory>, Ordered {
    ...
    public void customize(ConfigurableTomcatWebServerFactory factory) {
        ...
        tomcatProperties.getClass();
        propertyMapper.from(tomcatProperties::getMaxThreads).when((v1) -> {
            return isPositive(v1);
        }).to(maxThreads -> {
            customizeMaxThreads(factory, tomcatProperties.getMaxThreads());
        });
        tomcatProperties.getClass();
        propertyMapper.from(tomcatProperties::getMinSpareThreads).when((v1) -> {
            return isPositive(v1);
        }).to(minSpareThreads -> {
        ...
        tomcatProperties.getClass();
        propertyMapper.from(tomcatProperties::getMaxHttpPostSize).asInt((v0) -> {
            return v0.toBytes();
    ...
    private void customizeAcceptCount(ConfigurableTomcatWebServerFactory factory, int acceptCount) {
        factory.addConnectorCustomizers(connector -> {
            ...
            if (protocolHandler instanceof AbstractProtocol) {
                AbstractProtocol<?> protocol = protocolHandler;
    ...
    private void customizeMaxHttpHeaderSize(ConfigurableTomcatWebServerFactory factory, int maxHttpHeaderSize) {
        factory.addConnectorCustomizers(connector -> {
            ...
            if (protocolHandler instanceof AbstractHttp11Protocol) {
                AbstractHttp11Protocol protocol = protocolHandler;
