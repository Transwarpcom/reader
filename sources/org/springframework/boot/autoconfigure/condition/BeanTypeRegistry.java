...
/* loaded from: reader.jar:BOOT-INF/lib/spring-boot-autoconfigure-2.1.6.RELEASE.jar:org/springframework/boot/autoconfigure/condition/BeanTypeRegistry.class */
final class BeanTypeRegistry implements SmartInitializingSingleton {
    ...
    @FunctionalInterface
    /* loaded from: reader.jar:BOOT-INF/lib/spring-boot-autoconfigure-2.1.6.RELEASE.jar:org/springframework/boot/autoconfigure/condition/BeanTypeRegistry$TypeExtractor.class */
    ...
    public Set<String> getNamesForAnnotation(Class<? extends Annotation> annotation) {
        updateTypesIfNecessary();
        return (Set) this.beanTypes.entrySet().stream().filter(entry -> {
            return (entry.getValue() == null || AnnotationUtils.findAnnotation(((ResolvableType) entry.getValue()).resolve(), annotation) == null) ? false : true;
        }).map((v0) -> {
            return v0.getKey();
        }).collect(Collectors.toCollection(LinkedHashSet::new));
    }
    ...
    private void logIgnoredError(String message, String name, Exception ex) {
        if (logger.isDebugEnabled()) {
            logger.debug("Ignoring " + message + " '" + name + OperatorName.SHOW_TEXT_LINE, ex);
        }
    }
    ...
    private ResolvableType getDirectFactoryBeanGeneric(ConfigurableListableBeanFactory beanFactory, BeanDefinition definition) throws LinkageError, ClassNotFoundException {
    ...
    private ResolvableType getFactoryBeanType(BeanDefinition definition, ResolvableType type) throws LinkageError, ClassNotFoundException {
    ...
    private ResolvableType getTypeFromAttribute(Object attribute) throws LinkageError, ClassNotFoundException {
