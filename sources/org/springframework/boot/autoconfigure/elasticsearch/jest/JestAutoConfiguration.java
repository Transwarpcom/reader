...
/* loaded from: reader.jar:BOOT-INF/lib/spring-boot-autoconfigure-2.1.6.RELEASE.jar:org/springframework/boot/autoconfigure/elasticsearch/jest/JestAutoConfiguration.class */
public class JestAutoConfiguration {
    ...
    protected HttpClientConfig createHttpClientConfig() {
        ...
        map.from(jestProperties::getUsername).whenHasText().to(username -> {
        ...
        builder.getClass();
        sourceWhenNonNull.to(x$0 -> {
            builder.gson(x$0);
        });
        JestProperties jestProperties2 = this.properties;
        jestProperties2.getClass();
        PropertyMapper.Source sourceFrom = map.from(jestProperties2::isMultiThreaded);
        builder.getClass();
        sourceFrom.to(x$02 -> {
        ...
        PropertyMapper.Source<Integer> sourceAsInt = map.from(jestProperties3::getConnectionTimeout).whenNonNull().asInt((v0) -> {
            return v0.toMillis();
        });
        builder.getClass();
        sourceAsInt.to(x$03 -> {
        ...
        PropertyMapper.Source<Integer> sourceAsInt2 = map.from(jestProperties4::getReadTimeout).whenNonNull().asInt((v0) -> {
            return v0.toMillis();
        });
        builder.getClass();
        sourceAsInt2.to(x$04 -> {
