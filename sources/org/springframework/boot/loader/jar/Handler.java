...
/* loaded from: reader.jar:org/springframework/boot/loader/jar/Handler.class */
public class Handler extends URLStreamHandler {
    ...
    private URLConnection openConnection(URLStreamHandler handler, URL url) throws Exception {
        return new URL((URL) null, url.toExternalForm(), handler).openConnection();
    }
    ...
    private String getFileFromSpec(String spec) {
        ...
        if (separatorIndex == -1) {
            throw new IllegalArgumentException("No !/ in spec '" + spec + OperatorName.SHOW_TEXT_LINE);
        }
        try {
            new URL(spec.substring(0, separatorIndex));
            return spec;
        } catch (MalformedURLException ex) {
            throw new IllegalArgumentException("Invalid spec URL '" + spec + OperatorName.SHOW_TEXT_LINE, ex);
        }
    }
    ...
    private String getFileFromContext(URL context, String spec) {
        ...
        if (lastSlashIndex == -1) {
            throw new IllegalArgumentException("No / found in context URL's file '" + file + OperatorName.SHOW_TEXT_LINE);
        }
    ...
    private JarFile getRootJarFile(String name) throws IOException {
        try {
            ...
            throw new IOException("Unable to open root Jar file '" + name + OperatorName.SHOW_TEXT_LINE, ex);
        }
    }
