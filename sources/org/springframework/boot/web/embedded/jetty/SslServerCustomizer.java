...
/* loaded from: reader.jar:BOOT-INF/lib/spring-boot-2.1.6.RELEASE.jar:org/springframework/boot/web/embedded/jetty/SslServerCustomizer.class */
class SslServerCustomizer implements JettyServerCustomizer {
    ...
    private void configureSslKeyStore(SslContextFactory factory, Ssl ssl) {
        try {
            ...
            throw new WebServerException("Could not load key store '" + ssl.getKeyStore() + OperatorName.SHOW_TEXT_LINE, ex);
        }
    }
    ...
    private void configureSslTrustStore(SslContextFactory factory, Ssl ssl) {
        ...
        if (ssl.getTrustStore() != null) {
            try {
                ...
                throw new WebServerException("Could not find trust store '" + ssl.getTrustStore() + OperatorName.SHOW_TEXT_LINE, ex);
            }
        }
