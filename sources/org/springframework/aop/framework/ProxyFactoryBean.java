...
/* loaded from: reader.jar:BOOT-INF/lib/spring-aop-5.1.8.RELEASE.jar:org/springframework/aop/framework/ProxyFactoryBean.class */
public class ProxyFactoryBean extends ProxyCreatorSupport implements FactoryBean<Object>, BeanClassLoaderAware, BeanFactoryAware {
    ...
    private synchronized void initializeAdvisorChain() throws AopConfigException, BeansException {
        ...
        if (!ObjectUtils.isEmpty((Object[]) this.interceptorNames)) {
            ...
            for (String name : this.interceptorNames) {
                if (this.logger.isTraceEnabled()) {
                    this.logger.trace("Configuring advisor or advice '" + name + OperatorName.SHOW_TEXT_LINE);
                }
                if (name.endsWith("*")) {
    ...
    private List<Advisor> freshAdvisorChain() throws BeansException {
        ...
        for (Advisor advisor : advisors) {
            if (advisor instanceof PrototypePlaceholderAdvisor) {
                ...
                if (this.logger.isDebugEnabled()) {
                    this.logger.debug("Refreshing bean named '" + pa.getBeanName() + OperatorName.SHOW_TEXT_LINE);
                }
                if (this.beanFactory == null) {
                    throw new IllegalStateException("No BeanFactory available anymore (probably due to serialization) - cannot resolve prototype advisor '" + pa.getBeanName() + OperatorName.SHOW_TEXT_LINE);
                }
    ...
    private void addAdvisorOnChainCreation(Object next, String name) {
        ...
        if (this.logger.isTraceEnabled()) {
            this.logger.trace("Adding advisor with name '" + name + OperatorName.SHOW_TEXT_LINE);
        }
        addAdvisor(advisor);
    }
    ...
    private TargetSource freshTargetSource() throws BeansException {
        ...
        if (this.beanFactory == null) {
            throw new IllegalStateException("No BeanFactory available anymore (probably due to serialization) - cannot resolve target with name '" + this.targetName + OperatorName.SHOW_TEXT_LINE);
        }
        if (this.logger.isDebugEnabled()) {
            this.logger.debug("Refreshing target with name '" + this.targetName + OperatorName.SHOW_TEXT_LINE);
        }
    ...
    /* loaded from: reader.jar:BOOT-INF/lib/spring-aop-5.1.8.RELEASE.jar:org/springframework/aop/framework/ProxyFactoryBean$PrototypePlaceholderAdvisor.class */
    private static class PrototypePlaceholderAdvisor implements Advisor, Serializable {
        ...
        public PrototypePlaceholderAdvisor(String beanName) {
            ...
            this.message = "Placeholder for prototype Advisor/Advice with bean name '" + beanName + OperatorName.SHOW_TEXT_LINE;
        }
