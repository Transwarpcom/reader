...
/* loaded from: reader.jar:BOOT-INF/lib/spring-context-5.1.8.RELEASE.jar:org/springframework/context/event/EventListenerMethodProcessor.class */
            ...
            if (!ScopedProxyUtils.isScopedTarget(beanName)) {
                ...
                try {
                    ...
                    if (this.logger.isDebugEnabled()) {
                        this.logger.debug("Could not resolve target class for bean with name '" + beanName + OperatorName.SHOW_TEXT_LINE, ex);
                    }
                }
                if (type == null) {
                    continue;
                    ...
                    if (ScopedObject.class.isAssignableFrom(type)) {
                        try {
                            ...
                            if (this.logger.isDebugEnabled()) {
                                this.logger.debug("Could not resolve target bean for scoped proxy '" + beanName + OperatorName.SHOW_TEXT_LINE, ex2);
                            }
                        }
                    }
                    try {
                        ...
                        throw new BeanInitializationException("Failed to process @EventListener annotation on bean with name '" + beanName + OperatorName.SHOW_TEXT_LINE, ex3);
                    }
                }
            }
        }
    ...
    private void processBean(String beanName, Class<?> targetType) {
        if (!this.nonAnnotatedClasses.contains(targetType) && !targetType.getName().startsWith("java") && !isSpringContainerClass(targetType)) {
            ...
            try {
                ...
                if (this.logger.isDebugEnabled()) {
                    this.logger.debug("Could not resolve methods for bean with name '" + beanName + OperatorName.SHOW_TEXT_LINE, ex);
                }
            }
