...
/* loaded from: reader.jar:BOOT-INF/lib/spring-beans-5.1.8.RELEASE.jar:org/springframework/beans/factory/support/DisposableBeanAdapter.class */
class DisposableBeanAdapter implements DisposableBean, Runnable, Serializable {
        ...
        if (destroyMethodName != null && ((!this.invokeDisposableBean || !"destroy".equals(destroyMethodName)) && !beanDefinition.isExternallyManagedDestroyMethod(destroyMethodName))) {
            ...
            if (destroyMethod == null) {
                if (beanDefinition.isEnforceDestroyMethod()) {
                    throw new BeanDefinitionValidationException("Could not find a destroy method named '" + destroyMethodName + "' on bean with name '" + beanName + OperatorName.SHOW_TEXT_LINE);
                }
            } else {
    ...
    public void destroy() {
        ...
        if (this.invokeDisposableBean) {
            if (logger.isTraceEnabled()) {
                logger.trace("Invoking destroy() on bean with name '" + this.beanName + OperatorName.SHOW_TEXT_LINE);
            }
            try {
                ...
                String msg = "Invocation of destroy method failed on bean with name '" + this.beanName + OperatorName.SHOW_TEXT_LINE;
        ...
        } else if (this.destroyMethodName != null && (methodToInvoke = determineDestroyMethod(this.destroyMethodName)) != null) {
    ...
    private void invokeCustomDestroyMethod(Method destroyMethod) {
        ...
        if (logger.isTraceEnabled()) {
            logger.trace("Invoking destroy method '" + this.destroyMethodName + "' on bean with name '" + this.beanName + OperatorName.SHOW_TEXT_LINE);
        }
        try {
            if (System.getSecurityManager() != null) {
                AccessController.doPrivileged(() -> {
            ...
            logger.info("Failed to invoke destroy method '" + this.destroyMethodName + "' on bean with name '" + this.beanName + OperatorName.SHOW_TEXT_LINE, ex2);
        }
    }
