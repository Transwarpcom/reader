...
/* loaded from: reader.jar:BOOT-INF/lib/spring-beans-5.1.8.RELEASE.jar:org/springframework/beans/AbstractNestablePropertyAccessor.class */
public abstract class AbstractNestablePropertyAccessor extends AbstractPropertyAccessor {
    ...
    private void processKeyedProperty(PropertyTokenHolder tokens, PropertyValue pv) throws BeansException, ArrayIndexOutOfBoundsException, IllegalArgumentException, NegativeArraySizeException {
        ...
        if (propValue.getClass().isArray()) {
            ...
            try {
                ...
                throw new InvalidPropertyException(getRootClass(), this.nestedPath + tokens.canonicalName, "Invalid array index in property path '" + tokens.canonicalName + OperatorName.SHOW_TEXT_LINE, ex);
            }
        }
        if (propValue instanceof List) {
            ...
            try {
                ...
                throw new InvalidPropertyException(getRootClass(), this.nestedPath + tokens.canonicalName, "Invalid list index in property path '" + tokens.canonicalName + OperatorName.SHOW_TEXT_LINE, ex2);
            }
        }
    ...
    private Object getPropertyHoldingValue(PropertyTokenHolder tokens) throws BeansException {
        ...
        try {
            ...
            throw new NotWritablePropertyException(getRootClass(), this.nestedPath + tokens.canonicalName, "Cannot access indexed value in property referenced in indexed property path '" + tokens.canonicalName + OperatorName.SHOW_TEXT_LINE, ex);
        }
    }
                ...
                if (pv.isConverted()) {
                    ...
                    if (isExtractOldValueForEditor() && ph.isReadable()) {
                        try {
                            ...
                            if (logger.isDebugEnabled()) {
                                logger.debug("Could not read previous value of property '" + this.nestedPath + tokens.canonicalName + OperatorName.SHOW_TEXT_LINE, ex);
                            }
                        }
                    }
    ...
    protected Object getPropertyValue(PropertyTokenHolder tokens) throws BeansException, ArrayIndexOutOfBoundsException, IllegalArgumentException {
        ...
        try {
                ...
                for (int i = 0; i < tokens.keys.length; i++) {
                    ...
                    if (value.getClass().isArray()) {
                        ...
                        if (index3 < 0 || index3 >= set.size()) {
                            throw new InvalidPropertyException(getRootClass(), this.nestedPath + propertyName, "Cannot get element with index " + index3 + " from Set of size " + set.size() + ", accessed using property path '" + propertyName + OperatorName.SHOW_TEXT_LINE);
                        }
            ...
            throw new InvalidPropertyException(getRootClass(), this.nestedPath + propertyName, "Index of out of bounds in property path '" + propertyName + OperatorName.SHOW_TEXT_LINE, ex);
        } catch (NumberFormatException | TypeMismatchException ex2) {
            throw new InvalidPropertyException(getRootClass(), this.nestedPath + propertyName, "Invalid index in property path '" + propertyName + OperatorName.SHOW_TEXT_LINE, ex2);
    ...
    private AbstractNestablePropertyAccessor getNestedPropertyAccessor(String nestedProperty) throws BeansException, ArrayIndexOutOfBoundsException, IllegalArgumentException, NegativeArraySizeException {
        ...
        if (nestedPa == null || nestedPa.getWrappedInstance() != ObjectUtils.unwrapOptional(value)) {
            if (logger.isTraceEnabled()) {
                logger.trace("Creating new nested " + getClass().getSimpleName() + " for property '" + canonicalName + OperatorName.SHOW_TEXT_LINE);
            }
            ...
            logger.trace("Using cached nested property accessor for property '" + canonicalName + OperatorName.SHOW_TEXT_LINE);
        }
        return nestedPa;
    }
    ...
    private PropertyTokenHolder getPropertyNameTokens(String propertyName) {
        ...
        while (searchIndex != -1) {
            ...
            searchIndex = -1;
            if (keyStart != -1 && (keyEnd = propertyName.indexOf("]", keyStart + "[".length())) != -1) {
                if (actualName == null) {
                    actualName = propertyName.substring(0, keyStart);
                }
                String key = propertyName.substring(keyStart + "[".length(), keyEnd);
                if ((key.length() > 1 && key.startsWith(OperatorName.SHOW_TEXT_LINE) && key.endsWith(OperatorName.SHOW_TEXT_LINE)) || (key.startsWith(OperatorName.SHOW_TEXT_LINE_AND_SPACE) && key.endsWith(OperatorName.SHOW_TEXT_LINE_AND_SPACE))) {
    ...
    /* loaded from: reader.jar:BOOT-INF/lib/spring-beans-5.1.8.RELEASE.jar:org/springframework/beans/AbstractNestablePropertyAccessor$PropertyHandler.class */
    ...
    /* loaded from: reader.jar:BOOT-INF/lib/spring-beans-5.1.8.RELEASE.jar:org/springframework/beans/AbstractNestablePropertyAccessor$PropertyTokenHolder.class */
