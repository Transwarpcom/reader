...
/* loaded from: reader.jar:BOOT-INF/lib/rhino-1.7.13-1.jar:org/mozilla/javascript/optimizer/OptRuntime.class */
public final class OptRuntime extends ScriptRuntime {
    ...
    public static Object add(Object val1, double val2, Context cx) {
        if (val1 instanceof Double) {
            return Double.valueOf(((Double) val1).doubleValue() + val2);
        }
        if (val1 instanceof Integer) {
            return Double.valueOf(((Integer) val1).intValue() + val2);
        }
        return ScriptRuntime.add(val1, Double.valueOf(val2), cx);
    }

    public static Object add(double val1, Object val2, Context cx) {
        if (val2 instanceof Double) {
            return Double.valueOf(val1 + ((Double) val2).doubleValue());
        }
        if (val2 instanceof Integer) {
            return Double.valueOf(val1 + ((Integer) val2).intValue());
        }
        return ScriptRuntime.add(Double.valueOf(val1), val2, cx);
    }
    ...
    /* loaded from: reader.jar:BOOT-INF/lib/rhino-1.7.13-1.jar:org/mozilla/javascript/optimizer/OptRuntime$GeneratorState.class */
