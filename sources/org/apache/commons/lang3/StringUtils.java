...
/* loaded from: reader.jar:BOOT-INF/lib/commons-lang3-3.8.1.jar:org/apache/commons/lang3/StringUtils.class */
public class StringUtils {
        ...
        for (int i = 0; i < csLen; i++) {
            ...
            for (int j = 0; j < searchLen; j++) {
                if (searchChars[j] == ch2) {
                    if (i < csLast && j < searchLast && Character.isHighSurrogate(ch2)) {
    ...
    public static boolean containsAny(CharSequence cs, char... searchChars) {
        ...
        for (int i = 0; i < csLength; i++) {
            ...
            for (int j = 0; j < searchLength; j++) {
                if (searchChars[j] == ch2) {
                    if (!Character.isHighSurrogate(ch2) || j == searchLast) {
                        return true;
                    }
    ...
    */
    public static int indexOfAnyBut(java.lang.CharSequence r5, char... r6) {
        /*
            r0 = r5
    ...
    public static int indexOfAnyBut(CharSequence seq, CharSequence searchChars) {
        ...
        int strLen = seq.length();
        for (int i = 0; i < strLen; i++) {
            char ch2 = seq.charAt(i);
            boolean chFound = CharSequenceUtils.indexOf(searchChars, ch2, 0) >= 0;
            if (i + 1 < strLen && Character.isHighSurrogate(ch2)) {
                char ch22 = seq.charAt(i + 1);
                if (chFound && CharSequenceUtils.indexOf(searchChars, ch22, 0) < 0) {
                    return i;
                }
            } else if (!chFound) {
                return i;
    ...
    public static String capitalize(String str) {
        ...
        int iCharCount = Character.charCount(firstCodepoint);
        while (true) {
            int inOffset = iCharCount;
            if (inOffset < strLen) {
                int codepoint = str.codePointAt(inOffset);
                int i = outOffset;
                outOffset++;
                newCodePoints[i] = codepoint;
                iCharCount = inOffset + Character.charCount(codepoint);
            } else {
        ...
        while (true) {
            ...
            if (i < strLen) {
                ...
                if (Character.isUpperCase(oldCodepoint)) {
                    upperCase = Character.toLowerCase(oldCodepoint);
                } else if (Character.isTitleCase(oldCodepoint)) {
                    upperCase = Character.toLowerCase(oldCodepoint);
                } else if (Character.isLowerCase(oldCodepoint)) {
                    upperCase = Character.toUpperCase(oldCodepoint);
                } else {
                    upperCase = oldCodepoint;
                }
                int newCodePoint = upperCase;
                int i2 = outOffset;
                outOffset++;
                newCodePoints[i2] = newCodePoint;
                iCharCount = i + Character.charCount(newCodePoint);
            } else {
                return new String(newCodePoints, 0, outOffset);
            }
        }
        ...
        while (j <= m) {
            ...
            for (int i2 = min; i2 <= max; i2++) {
                if (s.charAt(i2 - 1) == t_j) {
                    ...
                    d[i2] = 1 + Math.min(Math.min(d[i2 - 1], p[i2]), p[i2 - 1]);
                }
            }
            int[] _d = p;
            p = d;
    ...
    public static String normalizeSpace(String str) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (isWhitespace) {
                ...
                int i3 = count;
                count++;
                newChars[i3] = actualChar == 160 ? ' ' : actualChar;
                whitespacesCount = 0;
            }
        }
        if (startWhitespaces) {
    ...
    public static String unwrap(String str, char wrapChar) {
        ...
        if (str.charAt(0) == wrapChar && str.charAt(str.length() - 1) == wrapChar && (endIndex = str.length() - 1) != -1) {
