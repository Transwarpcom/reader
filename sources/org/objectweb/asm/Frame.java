...
/* loaded from: reader.jar:BOOT-INF/lib/asm-9.1.jar:org/objectweb/asm/Frame.class */
class Frame {
    ...
    private static boolean merge(SymbolTable symbolTable, int sourceType, int[] dstTypes, int dstIndex) {
        ...
        if ((dstType & (-67108864)) != 0 || (dstType & KIND_MASK) == REFERENCE_KIND) {
            if (srcType == NULL) {
                return false;
            }
            if ((srcType & (-4194304)) == (dstType & (-4194304))) {
                if ((dstType & KIND_MASK) == REFERENCE_KIND) {
                    mergedType = (srcType & (-67108864)) | REFERENCE_KIND | symbolTable.addMergedType(srcType & VALUE_MASK, dstType & VALUE_MASK);
                } else {
                    int mergedDim = (-67108864) + (srcType & (-67108864));
                    mergedType = mergedDim | REFERENCE_KIND | symbolTable.addType("java/lang/Object");
                }
            } else if ((srcType & (-67108864)) != 0 || (srcType & KIND_MASK) == REFERENCE_KIND) {
            ...
            mergedType = ((srcType & (-67108864)) != 0 || (srcType & KIND_MASK) == REFERENCE_KIND) ? srcType : 4194304;
        } else {
            mergedType = 4194304;
        }
