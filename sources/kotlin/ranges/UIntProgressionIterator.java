...
/* loaded from: reader.jar:BOOT-INF/lib/kotlin-stdlib-1.5.21.jar:kotlin/ranges/UIntProgressionIterator.class */
final class UIntProgressionIterator extends UIntIterator {
    ...
    private UIntProgressionIterator(int first, int last, int step) {
        ...
        if (step > 0) {
            z = UnsignedKt.uintCompare(first, last) <= 0;
        } else {
            z = UnsignedKt.uintCompare(first, last) >= 0;
        }
        this.hasNext = z;
        this.step = UInt.m2244constructorimpl(step);
    ...
    public int mo2268nextUIntpVg5ArA() {
        ...
        if (value == this.finalElement) {
            ...
            this.next = UInt.m2244constructorimpl(this.next + this.step);
        }
        return value;
    }
}
