...
/* compiled from: EncodingDetectHelp.java */
/* loaded from: reader.jar:BOOT-INF/classes/io/legado/app/help/BytesEncodingDetect.class */
class BytesEncodingDetect extends Encoding {
    ...
    public int detectEncoding(URL testurl) throws IOException {
        int guess;
    ...
    public static byte[] getFileBytes(File testfile) throws IOException {
    ...
    int gb2312_probability(byte[] rawtext) {
        ...
        float rangeval = 50.0f * (gbchars / dbchars);
        float freqval = 50.0f * (gbfreq / totalfreq);
        ...
        while (i < rawtextlen - 1) {
            if (rawtext[i] < 0) {
                ...
                if (-95 <= rawtext[i] && rawtext[i] <= -9 && -95 <= rawtext[i + 1] && rawtext[i + 1] <= -2) {
                    ...
                    totalfreq += 500;
                    int row = (rawtext[i] + 256) - 161;
                    int column2 = (rawtext[i + 1] + 256) - 161;
                    if (this.GBFreq[row][column2] != 0) {
                        gbfreq += this.GBFreq[row][column2];
                    } else if (15 <= row && row < 55) {
                        gbfreq += 200;
                    }
                    ...
                    gbchars++;
                    totalfreq += 500;
                    int row2 = (rawtext[i] + 256) - 129;
                    if (64 <= rawtext[i + 1] && rawtext[i + 1] <= 126) {
                        column = rawtext[i + 1] - 64;
                    } else {
                        column = (rawtext[i + 1] + 256) - 64;
                    }
                    if (this.GBKFreq[row2][column] != 0) {
                        gbfreq += this.GBKFreq[row2][column];
                    }
                }
                i++;
            }
    ...
    int hz_probability(byte[] rawtext) {
        ...
        while (i < rawtextlen) {
            if (rawtext[i] == 126) {
                if (rawtext[i + 1] == 123) {
                    ...
                    while (true) {
                        i += 2;
                        if (i < rawtextlen - 1 && rawtext[i] != 10 && rawtext[i] != 13) {
                            ...
                            if (33 <= rawtext[i] && rawtext[i] <= 119 && 33 <= rawtext[i + 1] && rawtext[i + 1] <= 119) {
                                ...
                                hzchars += 2;
                                int row2 = (rawtext[i] + 256) - 161;
                                int column2 = (rawtext[i + 1] + 256) - 161;
                                totalfreq += 500;
                                if (this.GBFreq[row2][column2] != 0) {
                                    hzfreq += this.GBFreq[row2][column2];
                                } else if (15 <= row2 && row2 < 55) {
                                    hzfreq += 200;
                                }
                            }
                            dbchars += 2;
        ...
        float freqval = 50.0f * (hzfreq / totalfreq);
        return (int) (rangeval + freqval);
    }
    ...
    int big5_probability(byte[] rawtext) {
        ...
        float rangeval = 50.0f * (bfchars / dbchars);
        float freqval = 50.0f * (bffreq / totalfreq);
        return (int) (rangeval + freqval);
    }
        ...
        while (i < rawtextlen - 1) {
            if (rawtext[i] < 128) {
                ...
                if (161 <= rawtext[i] && rawtext[i] <= 249 && ((64 <= rawtext[i + 1] && rawtext[i + 1] <= 126) || (161 <= rawtext[i + 1] && rawtext[i + 1] <= 254))) {
                    ...
                    if (64 <= rawtext[i + 1] && rawtext[i + 1] <= 126) {
                        column2 = rawtext[i + 1] - 64;
                    } else {
                        column2 = rawtext[i + 1] - 97;
                    }
                    if (this.Big5Freq[row][column2] != 0) {
                        bffreq += this.Big5Freq[row][column2];
                    } else if (3 <= row && row < 37) {
                        bffreq += 200;
                    }
                    ...
                    bfchars++;
                    totalfreq += 500;
                    int row2 = rawtext[i] - 129;
                    if (64 > rawtext[i + 1] || rawtext[i + 1] <= 126) {
                        column = rawtext[i + 1] - 64;
                    } else {
                        column = rawtext[i + 1] - 64;
                    }
                    if (this.Big5PFreq[row2][column] != 0) {
                        bffreq += this.Big5PFreq[row2][column];
                    }
                }
                i++;
            }
    ...
    int euc_tw_probability(byte[] rawtext) {
        ...
        float rangeval = 50.0f * (cnschars / dbchars);
        float freqval = 50.0f * (cnsfreq / totalfreq);
        return (int) (rangeval + freqval);
    }
    ...
    int iso_2022_cn_probability(byte[] rawtext) {
                ...
                if (rawtext[i + 1] == 36 && rawtext[i + 2] == 41 && rawtext[i + 3] == 65) {
                    ...
                    while (rawtext[i] != 27) {
                        ...
                        if (33 <= rawtext[i] && rawtext[i] <= 126 && 33 <= rawtext[i + 1] && rawtext[i + 1] <= 126) {
                            isochars++;
                            totalfreq += 500;
                            int row2 = rawtext[i] - 33;
                            int column2 = rawtext[i + 1] - 33;
                            if (this.EUC_TWFreq[row2][column2] != 0) {
                                isofreq += this.EUC_TWFreq[row2][column2];
                            } else if (35 <= row2 && row2 <= 92) {
                                isofreq += 150;
                            }
                            i++;
                        }
        ...
        float rangeval = 50.0f * (isochars / dbchars);
        float freqval = 50.0f * (isofreq / totalfreq);
        return (int) (rangeval + freqval);
    }
    ...
    int utf8_probability(byte[] rawtext) {
        ...
        int score = (int) (100.0f * (goodbytes / (rawtextlen - asciibytes)));
        if (score > 98) {
            return score;
        }
    ...
    int euc_kr_probability(byte[] rawtext) {
        ...
        float rangeval = 50.0f * (krchars / dbchars);
        float freqval = 50.0f * (krfreq / totalfreq);
        return (int) (rangeval + freqval);
    }
    ...
    int euc_jp_probability(byte[] rawtext) {
        ...
        float rangeval = 50.0f * (jpchars / dbchars);
        float freqval = 50.0f * (jpfreq / totalfreq);
        return (int) (rangeval + freqval);
    }
        ...
        while (i < rawtextlen - 1) {
            if (rawtext[i] < 0) {
                ...
                if (i + 1 < rawtext.length && (((-127 <= rawtext[i] && rawtext[i] <= -97) || (-32 <= rawtext[i] && rawtext[i] <= -17)) && ((64 <= rawtext[i + 1] && rawtext[i + 1] <= 126) || (-128 <= rawtext[i + 1] && rawtext[i + 1] <= -4)))) {
                    ...
                    if (row2 < 160) {
                        row = ((row2 - 112) << 1) - adjust;
                    } else {
                        row = ((row2 - 176) << 1) - adjust;
                    }
                    int row3 = row - 32;
                    if (row3 < this.JPFreq.length && 32 < this.JPFreq[row3].length && this.JPFreq[row3][32] != 0) {
                        jpfreq += this.JPFreq[row3][32];
                    }
                    i++;
    ...
    void initialize_frequencies() {
        ...
        for (int i2 = 125; i2 >= 0; i2--) {
            for (int j2 = 190; j2 >= 0; j2--) {
                this.GBKFreq[i2][j2] = 0;
            }
        }
        for (int i3 = 93; i3 >= 0; i3--) {
            for (int j3 = 157; j3 >= 0; j3--) {
                this.Big5Freq[i3][j3] = 0;
            }
        }
        for (int i4 = 125; i4 >= 0; i4--) {
            for (int j4 = 190; j4 >= 0; j4--) {
                this.Big5PFreq[i4][j4] = 0;
            }
        }
        for (int i5 = 93; i5 >= 0; i5--) {
            for (int j5 = 93; j5 >= 0; j5--) {
                this.EUC_TWFreq[i5][j5] = 0;
            }
        }
        for (int i6 = 93; i6 >= 0; i6--) {
            for (int j6 = 93; j6 >= 0; j6--) {
                this.JPFreq[i6][j6] = 0;
            }
        }
