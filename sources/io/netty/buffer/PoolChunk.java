...
/* loaded from: reader.jar:BOOT-INF/lib/netty-buffer-4.1.42.Final.jar:io/netty/buffer/PoolChunk.class */
final class PoolChunk<T> implements PoolChunkMetric {
    ...
    void free(long handle, ByteBuffer nioBuffer) {
        ...
        if (bitmapIdx != 0) {
            ...
            synchronized (head) {
                if (subpage.free(head, bitmapIdx & LockFreeTaskQueueCore.MAX_CAPACITY_MASK)) {
                    return;
                }
            }
        }
    ...
    void initBuf(PooledByteBuf<T> buf, ByteBuffer nioBuffer, long handle, int reqCapacity) {
        ...
        if (bitmapIdx == 0) {
            ...
            buf.init(this, nioBuffer, handle, runOffset(memoryMapIdx) + this.offset, reqCapacity, runLength(memoryMapIdx), this.arena.parent.threadCache());
            return;
        }
    ...
    private void initBufWithSubpage(PooledByteBuf<T> buf, ByteBuffer nioBuffer, long handle, int bitmapIdx, int reqCapacity) {
        ...
        buf.init(this, nioBuffer, handle, runOffset(memoryMapIdx) + ((bitmapIdx & LockFreeTaskQueueCore.MAX_CAPACITY_MASK) * subpage.elemSize) + this.offset, reqCapacity, subpage.elemSize, this.arena.parent.threadCache());
    }
