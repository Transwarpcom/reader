...
/* loaded from: reader.jar:BOOT-INF/lib/netty-buffer-4.1.42.Final.jar:io/netty/buffer/UnsafeByteBufUtil.class */
final class UnsafeByteBufUtil {
    ...
    static int getUnsignedMedium(long address) {
        ...
        if (UNALIGNED) {
            ...
            if (PlatformDependent.BIG_ENDIAN_NATIVE_ORDER) {
                sReverseBytes = PlatformDependent.getShort(address + 1);
            } else {
                sReverseBytes = Short.reverseBytes(PlatformDependent.getShort(address + 1));
            }
            return i | (sReverseBytes & 65535);
        }
    ...
    static int getUnsignedMediumLE(long address) {
        ...
        if (UNALIGNED) {
            ...
            return i | ((sReverseBytes & 65535) << 8);
        }
    ...
    static long getLong(long address) {
        ...
        return (PlatformDependent.getByte(address) << 56) | ((PlatformDependent.getByte(address + 1) & 255) << 48) | ((PlatformDependent.getByte(address + 2) & 255) << 40) | ((PlatformDependent.getByte(address + 3) & 255) << 32) | ((PlatformDependent.getByte(address + 4) & 255) << 24) | ((PlatformDependent.getByte(address + 5) & 255) << 16) | ((PlatformDependent.getByte(address + 6) & 255) << 8) | (PlatformDependent.getByte(address + 7) & 255);
    }
    ...
    static long getLongLE(long address) {
        ...
        return (PlatformDependent.getByte(address) & 255) | ((PlatformDependent.getByte(address + 1) & 255) << 8) | ((PlatformDependent.getByte(address + 2) & 255) << 16) | ((PlatformDependent.getByte(address + 3) & 255) << 24) | ((PlatformDependent.getByte(address + 4) & 255) << 32) | ((PlatformDependent.getByte(address + 5) & 255) << 40) | ((PlatformDependent.getByte(address + 6) & 255) << 48) | (PlatformDependent.getByte(address + 7) << 56);
    }
    ...
    static int getUnsignedMedium(byte[] array, int index) {
        ...
        if (UNALIGNED) {
            ...
            if (PlatformDependent.BIG_ENDIAN_NATIVE_ORDER) {
                sReverseBytes = PlatformDependent.getShort(array, index + 1);
            } else {
                sReverseBytes = Short.reverseBytes(PlatformDependent.getShort(array, index + 1));
            }
            return i | (sReverseBytes & 65535);
        }
    ...
    static void setBytes(AbstractByteBuf buf, long addr, int index, ByteBuf src, int srcIndex, int length) {
        buf.checkIndex(index, length);
        ObjectUtil.checkNotNull(src, NCXDocumentV2.NCXAttributes.src);
    ...
    static void setBytes(AbstractByteBuf buf, long addr, int index, ByteBuffer src) {
        ...
        if (src.hasArray()) {
            ...
            if (!$assertionsDisabled && buf.nioBufferCount() != 1) {
