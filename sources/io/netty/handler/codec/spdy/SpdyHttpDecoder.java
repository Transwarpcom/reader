...
/* loaded from: reader.jar:BOOT-INF/lib/netty-codec-http-4.1.42.Final.jar:io/netty/handler/codec/spdy/SpdyHttpDecoder.class */
public class SpdyHttpDecoder extends MessageToMessageDecoder<SpdyFrame> {
    ...
    private static FullHttpRequest createHttpRequest(SpdyHeadersFrame requestFrame, ByteBufAllocator alloc) throws Exception {
        ...
        try {
            ...
            req.headers().mo1933remove(HttpHeaderNames.TRANSFER_ENCODING);
            release = false;
            if (0 != 0) {
                buffer.release();
            }
    ...
    private static FullHttpResponse createHttpResponse(SpdyHeadersFrame responseFrame, ByteBufAllocator alloc, boolean validateHeaders) throws Exception {
        ...
        try {
            ...
            res.headers().mo1933remove(HttpHeaderNames.TRANSFER_ENCODING);
            res.headers().mo1933remove(HttpHeaderNames.TRAILER);
            release = false;
            if (0 != 0) {
                buffer.release();
            }
