...
/* loaded from: reader.jar:BOOT-INF/lib/netty-codec-4.1.42.Final.jar:io/netty/handler/codec/compression/LzmaFrameEncoder.class */
public class LzmaFrameEncoder extends MessageToByteEncoder<ByteBuf> {
    ...
    public LzmaFrameEncoder(int lc, int lp, int pb, int dictionarySize, boolean endMarkerMode, int numFastBytes) {
        ...
        if (numFastBytes < 5 || numFastBytes > MAX_FAST_BYTES) {
            throw new IllegalArgumentException(String.format("numFastBytes: %d (expected: %d-%d)", Integer.valueOf(numFastBytes), 5, Integer.valueOf(MAX_FAST_BYTES)));
        }
