...
/* loaded from: reader.jar:BOOT-INF/lib/netty-resolver-dns-4.1.42.Final.jar:io/netty/resolver/dns/DnsNameResolver.class */
public class DnsNameResolver extends InetNameResolver {
    ...
    protected void doResolveAll(String inetHost, DnsRecord[] additionals, Promise<List<InetAddress>> promise, DnsCache resolveCache) throws Exception {
        ...
        if (hostsFileEntry != null) {
            ...
            doResolveAllUncached(hostname, additionals, promise, resolveCache, this.completeOncePreferredResolved);
        }
    }
    ...
    private void doResolveAllUncached(final String hostname, final DnsRecord[] additionals, final Promise<List<InetAddress>> promise, final DnsCache resolveCache, final boolean completeEarlyIfPossible) {
        ...
        if (executor.inEventLoop()) {
            ...
            executor.execute(new Runnable() { // from class: io.netty.resolver.dns.DnsNameResolver.6
                ...
                public void run() {
                    DnsNameResolver.this.doResolveAllUncached0(hostname, additionals, promise, resolveCache, completeEarlyIfPossible);
                }
            });
        }
    }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void doResolveAllUncached0(String hostname, DnsRecord[] additionals, Promise<List<InetAddress>> promise, DnsCache resolveCache, boolean completeEarlyIfPossible) {
        if (!$assertionsDisabled && !executor().inEventLoop()) {
            throw new AssertionError();
        }
        DnsServerAddressStream nameServerAddrs = this.dnsServerAddressStreamProvider.nameServerAddressStream(hostname);
        new DnsAddressResolveContext(this, hostname, additionals, nameServerAddrs, resolveCache, this.authoritativeDnsServerCache, completeEarlyIfPossible).resolve(promise);
    }
    ...
    /* loaded from: reader.jar:BOOT-INF/lib/netty-resolver-dns-4.1.42.Final.jar:io/netty/resolver/dns/DnsNameResolver$DnsResponseHandler.class */
    private final class DnsResponseHandler extends ChannelInboundHandlerAdapter {
        ...
        public void channelRead(ChannelHandlerContext ctx, Object msg) {
            try {
                ...
                if (qCtx == null) {
                    DnsNameResolver.logger.warn("{} Received a DNS response with an unknown ID: {}", DnsNameResolver.this.f7ch, Integer.valueOf(queryId));
                ...
                if (res.isTruncated() && DnsNameResolver.this.socketChannelFactory != null) {
                    ...
                    bs.option(ChannelOption.SO_REUSEADDR, true).group(DnsNameResolver.this.executor()).channelFactory(DnsNameResolver.this.socketChannelFactory).handler(new ChannelInitializer<Channel>() { // from class: io.netty.resolver.dns.DnsNameResolver.DnsResponseHandler.1
                        @Override // io.netty.channel.ChannelInitializer
                        protected void initChannel(Channel ch2) {
                            ch2.pipeline().addLast(DnsNameResolver.TCP_ENCODER);
                            ch2.pipeline().addLast(new TcpDnsResponseDecoder());
                            ch2.pipeline().addLast(new ChannelInboundHandlerAdapter() { // from class: io.netty.resolver.dns.DnsNameResolver.DnsResponseHandler.1.1
                                ...
                                public void channelRead(ChannelHandlerContext ctx2, Object msg2) {
                                    try {
                                        ...
                                        if (DnsNameResolver.logger.isDebugEnabled()) {
                                            DnsNameResolver.logger.debug("{} RECEIVED: TCP [{}: {}], {}", channel, Integer.valueOf(queryId2), channel.remoteAddress(), response);
                                        }
                                        DnsQueryContext tcpCtx = DnsNameResolver.this.queryContextManager.get(res.sender(), queryId2);
                                        if (tcpCtx == null) {
                                            DnsNameResolver.logger.warn("{} Received a DNS response with an unknown ID: {}", channel, Integer.valueOf(queryId2));
                                            qCtx.finish(res);
                                            ReferenceCountUtil.release(msg2);
                                        } else {
                                            res.release();
                                            tcpCtx.finish(DnsNameResolver.this.new AddressedEnvelopeAdapter((InetSocketAddress) ctx2.channel().remoteAddress(), (InetSocketAddress) ctx2.channel().localAddress(), response));
                                            this.finish = true;
                                            ReferenceCountUtil.release(msg2);
                                        }
                                    } catch (Throwable th) {
                    ...
                    bs.connect(res.sender()).addListener2((GenericFutureListener<? extends Future<? super Void>>) new ChannelFutureListener() { // from class: io.netty.resolver.dns.DnsNameResolver.DnsResponseHandler.2
                        @Override // io.netty.util.concurrent.GenericFutureListener
                        public void operationComplete(ChannelFuture future) {
                            if (!future.isSuccess()) {
                                ...
                                new TcpDnsQueryContext(DnsNameResolver.this, channel, (InetSocketAddress) channel.remoteAddress(), qCtx.question(), DnsNameResolver.EMPTY_ADDITIONALS, promise).query(true, future.channel().newPromise());
                                promise.addListener2(new FutureListener<AddressedEnvelope<DnsResponse, InetSocketAddress>>() { // from class: io.netty.resolver.dns.DnsNameResolver.DnsResponseHandler.2.1
                                    @Override // io.netty.util.concurrent.GenericFutureListener
        ...
        public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
            DnsNameResolver.logger.warn("{} Unexpected exception: ", ctx.channel(), cause);
        }
    }

    /* loaded from: reader.jar:BOOT-INF/lib/netty-resolver-dns-4.1.42.Final.jar:io/netty/resolver/dns/DnsNameResolver$AddressedEnvelopeAdapter.class */
