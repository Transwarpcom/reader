...
/* loaded from: reader.jar:BOOT-INF/lib/guava-28.0-jre.jar:com/google/common/util/concurrent/Uninterruptibles.class */
public final class Uninterruptibles {
    ...
    public static boolean awaitUninterruptibly(CountDownLatch latch, Duration timeout) {
        return awaitUninterruptibly(latch, Internal.saturatedToNanos(timeout), TimeUnit.NANOSECONDS);
    }
    ...
    public static boolean awaitUninterruptibly(Condition condition, Duration timeout) {
        return awaitUninterruptibly(condition, Internal.saturatedToNanos(timeout), TimeUnit.NANOSECONDS);
    }
    ...
    public static boolean awaitUninterruptibly(Condition condition, long timeout, TimeUnit unit) {
        ...
        try {
            ...
            while (true) {
                try {
                    zAwait = condition.await(remainingNanos, TimeUnit.NANOSECONDS);
                    break;
                } catch (InterruptedException e) {
                    interrupted = true;
                    remainingNanos = end - System.nanoTime();
    ...
    public static void joinUninterruptibly(Thread toJoin, Duration timeout) {
        joinUninterruptibly(toJoin, Internal.saturatedToNanos(timeout), TimeUnit.NANOSECONDS);
    }
    ...
    public static <V> V getUninterruptibly(Future<V> future, Duration duration) throws ExecutionException, TimeoutException {
        return (V) getUninterruptibly(future, Internal.saturatedToNanos(duration), TimeUnit.NANOSECONDS);
    }
    ...
    public static void sleepUninterruptibly(Duration sleepFor) {
        sleepUninterruptibly(Internal.saturatedToNanos(sleepFor), TimeUnit.NANOSECONDS);
    }
    ...
    public static boolean tryAcquireUninterruptibly(Semaphore semaphore, Duration timeout) {
        return tryAcquireUninterruptibly(semaphore, Internal.saturatedToNanos(timeout), TimeUnit.NANOSECONDS);
    }
    ...
    public static boolean tryAcquireUninterruptibly(Semaphore semaphore, int permits, Duration timeout) {
        return tryAcquireUninterruptibly(semaphore, permits, Internal.saturatedToNanos(timeout), TimeUnit.NANOSECONDS);
    }
