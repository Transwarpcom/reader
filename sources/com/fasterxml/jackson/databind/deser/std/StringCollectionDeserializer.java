...
@JacksonStdImpl
/* loaded from: reader.jar:BOOT-INF/lib/jackson-databind-2.9.9.jar:com/fasterxml/jackson/databind/deser/std/StringCollectionDeserializer.class */
        ...
        while (true) {
            if (p.nextTextValue() == null) {
                ...
                if (t != JsonToken.END_ARRAY) {
                    if (t == JsonToken.VALUE_NULL) {
                        if (!this._skipNullValues) {
                            objDeserialize = this._nullProvider.getNullValue(ctxt);
                        }
                    } else {
                        objDeserialize = deser.deserialize(p, ctxt);
                    }
                } else {
                    return result;
                }
            ...
            String value = (String) objDeserialize;
            result.add(value);
        }
    }
